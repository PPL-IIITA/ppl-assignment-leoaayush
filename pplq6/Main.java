...
public class Main {
	...
	//		bw.write(Integer.toString(k));
		...
		//	Boys b1;
			...
			File fp1=new File("Output6.txt");
		    Boolean check1;
			check1=fp.createNewFile();
			FileWriter fw1=new FileWriter("Output6.txt");
	...
	//		bw1.write("Gifting Description");
		//	bw1.newLine();
			...
			for(i=0;i<k;i++)
			{
				c[i].happiness=c[i].bhap + c[i].ghap -10000 ;
	...
	//		bw1.newLine();
		...
		//	for(i=0;i<k;i++)
		//	{
		//		if(c[i].happiness<2 && c[i].hflag==0)
		//		{
			//		min=c[i].happiness;
			//		minindex=i;
		//		}
		...
		//		break;
		//	m++;
		//	System.out.println(c[p].happiness);
			if( c[p].happiness<= t){
			bw.write(c[p].name1 +"is breaking up with  "+c[p].name2);
			bw.newLine();
			//log FILE ENTRY
			c[p].name1=" ";
			...
			{ d=0;
				if(b[q].flagb==0)
				{
					if(b[q].bud >= c[p].maintg && b[q].minattract <=c[p].gatrr)
					{
						d=1;
					...
					if(d==1 ){
						...
						c[p].battr=b[q].attb;
										c[p].bintl=b[q].intelb;
		//								bw1.newLine();
						//				if(c[p].hflag==0){
										bw1.write(c[p].name1 +" is in a new relationship with  "+c[p].name2);
							//			}
					//				c[p].hflag=1;		
										break;				
					}
	...
	//		c[p].hflag=1;	
		...
		bw1.write("Day " + z1 +" over------------------------------------");
		bw1.newLine();
		bw.write("Day " + z1 +" over------------------------------------");
		bw.newLine();
	}
